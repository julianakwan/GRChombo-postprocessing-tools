...
std::string
SmallDataIOReader::read_entire_file(const std::string a_filename) const
{

    std::string warning{"Failed to open file \n"};

    if (std::ifstream is{a_filename, std::ios::binary | std::ios::ate})
    {
        auto size = is.tellg();
        ...
        is.seekg(0);
        if (is.read(&file_contents[0], size))
        {
...
void SmallDataIOReader::determine_file_structure()
{
    ...
    int current_position = file_stream.tellg();
    ...
    for (int irow = 0; irow < m_file_structure.num_data_rows[a_block]; ++irow)
    {
             ...
             icolumn < m_file_structure.num_data_columns[a_block]; ++icolumn)
        {
            if (a_min_column <= icolumn && icolumn < a_max_column)
            {
                ...
                std::cout << out[icolumn - a_min_column][irow] << std::endl;
            }
            else
                file_stream >> discard;
        }
                               ...
                               const int a_block)
{
    ...
    std::vector<std::string> indiv_names;

    while (!name_stream.eof())
    {
        std::string name;
        name_stream >> name;
        indiv_names.push_back(name);
    }
    ...
    for (int i = 0; i < indiv_names.size(); ++i)
    {
        for (int j = 0; j < m_file_structure.num_data_columns[a_block]; ++j)
        {
            std::string current_name = header[j];
            if (current_name.find(indiv_names[i]) == 0)
            {
                column_numbers.push_back(j);
                std::cout << "Found column name " << indiv_names[i]
                          << std::endl;
                break;
            }
        }
